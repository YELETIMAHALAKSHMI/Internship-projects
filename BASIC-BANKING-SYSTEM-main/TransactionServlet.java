...
String usr1 = req.getParameter("name1");
String amt = req.getParameter("amount");
String usr2 = req.getParameter("name2");
...
ResultSet rs = stm.executeQuery("select Balance from customer where Name='"+usr1+"'");

if(rs.next()){
String n = rs.getString(1);
int balance1 = Integer.parseInt(n);
if(balance1 - amount > 0){
String q = "update customer set  Balance = Balance +'"+amount+"' where name='"+usr2+"'";
int x = stm.executeUpdate(q);
String q1 = "update customer set Balance = Balance - '"+amount+"' where name='"+usr1+"'";
...
out.println("<center><h1>Transaction Successful...</h1></center>");
out.println("<h1><center><a href = 'home1.html'>Cilck this to go back <---</a></center></h1>");
String q2 = "insert into transaction values('"+usr1+"','"+usr2+"',"+amount+",'"+dtf.format(now)+"')";
...
out.println("<h1><center><a href = 'home1.html'>Cilck this to go back <---</a></center></h1>");
String q2 = "insert into transaction values('"+usr1+"','"+usr2+"',"+amount+",'"+dtf.format(now)+" (Failed) "+"')";
PreparedStatement stm1 = con.prepareStatement(q2);
int x1 = stm1.executeUpdate();
}
}
